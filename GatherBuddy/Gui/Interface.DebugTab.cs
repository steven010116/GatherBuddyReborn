...
public partial class Interface
{
    ...
    private static unsafe void DrawDebugFishingState()
    {
        ...
        ImGuiUtil.DrawTableColumn(
            $"{_plugin.FishRecorder.LastCatch?.Name[ClientLanguage.ChineseTraditional] ?? "None"} ({_plugin.FishRecorder.LastCatch?.ItemId ?? 0} - {_plugin.FishRecorder.LastCatch?.FishId ?? 0})");
        ImGuiUtil.DrawTableColumn("Current Catch");
        ImGuiUtil.DrawTableColumn(
            $"{record.Catch?.Name[ClientLanguage.ChineseTraditional] ?? "None"} ({record.Catch?.ItemId ?? 0} - {record.Catch?.FishId ?? 0}) - of size {record.Size / 10f} times {record.Amount}");
    ...
    private unsafe void DrawDebugFishingTimes()
    {
        ...
        {
            ImGuiUtil.DrawTableColumn(GatherBuddy.GameData.Fishes[fishId].Name[ClientLanguage.ChineseTraditional]);
    ...
    private static void DrawUptimeManagerTable()
    {
        ...
        foreach (var item in GatherBuddy.GameData.Gatherables.Values)
        {
            ...
            ImGuiUtil.DrawTableColumn(item.Name[ClientLanguage.ChineseTraditional]);
        ...
        foreach (var fish in GatherBuddy.GameData.Fishes.Values)
        {
            ...
            ImGuiUtil.DrawTableColumn(fish.Name[ClientLanguage.ChineseTraditional]);
    ...
    private void DrawAlarmDebug()
    {
        ...
        {
            ImGuiUtil.DrawTableColumn(alarm.Name.Any() ? alarm.Name : alarm.Item.Name[ClientLanguage.ChineseTraditional]);
